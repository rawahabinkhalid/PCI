...
if($_SERVER['REQUEST_METHOD']=='POST') {
    ...
    $sql = 'SELECT * FROM tutorform_section1 WHERE TeacherId = ' . $jsonObj['TutorId'];
    ...
    if($result->num_rows > 0) {
        ...
        $areaOfInterested = [];
        $sql1 = 'SELECT * FROM tutorform_section4_locations WHERE UserId = ' . $row['Id'];
        $result1 = $conn->query($sql1);
        if($result1->num_rows > 0) {
            while($row1 = $result1->fetch_assoc()) {
                $areaOfInterested[] = $row1;
            }
        }
        $row['AreaOfInterested'] = $areaOfInterested;
