...
<body class="hold-transition sidebar-mini">
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            <div class="content">
                <div class="container-fluid">
                    <form action="searchtutorsubmit.php" id="studentform">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <p style="font-weight: bold; margin-bottom:5px;">Search Tuition For:</p>
                                    <select class="form-control" name="searchchildren[]" required style="outline: none;"
                                        multiple id="childrens">
                                        <?php
    ...
    $(document).ready(function() {
        $('#childrens').multiselect({
            nonSelectedText: 'Select Childrens',
    ...
    $("#studentform").submit(function(e) {
        ...
        var data = $('#studentform').serializeJSON();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                content = '';
                content +=
                    '<table id="myTable"><thead><tr><th>S.No</th><th>Profile</th><th>Name</th><th>Preffered Subjects</th><th>Qualification</th><th>Address</th><th>Age</th><th>Experience</th><th>Status</th></tr></thead>';
                content += '<tbody>';
                for (i = 0; i < obj.length; i++) {
                    ...
                    if (obj[i].Status == 'null' || obj[i].Status == 'Rejected') {
                        content +=
                            '<button class="btn btn-info requestDemo" type="button" value="' + obj[
                                i]
    ...
    $(document).on('click', '.requestDemo', function() {
        var children = [];
        $("#childrens > option:selected").each(function() {
            children.push(this.value);
        });

        var obj = {};
        obj.TeacherId = $(this).val();
        obj.Students = children;
        console.log(obj);
        $.ajax({
            type: "POST",
            url: "RequestDemo.php",
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (i = 0; i < obj.length; i++) {
                    ...
                    if (obj[i].Status == 'null' || obj[i].Status == 'Rejected') {
                        content +=
                            '<button class="btn btn-info requestDemo" type="button" value="' +
